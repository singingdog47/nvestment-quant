...
- 市場参加の広がり：-2.2ポイント
- 流動性：+8.6ポイント
- Tsakos Energy Navigation Ltd Common Shares、Vertex Corporationの順位上昇が目立ちます。材料と現在価格を確認する優先候補です。

## 今の相場を人間の言葉で

日本はFinancialsが10銘柄。米国はFinancialsが6銘柄で、上位銘柄に偏りがあります。市場のストレスと参加の広がりを見ながら、個別銘柄を選別する局面です。
米国債金利の実現ボラproxyは75パーセンタイルで、ICE MOVEとは別の公式米財務省データによる参考値です。
同じテーマの上位銘柄を複数買うと、銘柄数が増えても実質的な分散にならない点に注意してください。

## SQ・短期需給

メジャーSQ（2026-09-11）まで0日。執行警戒度は3.8/15で軽度です。戦略を変えるほどではありませんが、成行で価格を追う必要はありません。 建玉・ロール等の詳細データはpartial（確信度25%）なので、上か下かの方向は推定しません。
...
- **WATCH** Apple Watch Series 12、Apple Watch Ultra 4を含むApple Watch 5機種がau Starlink Directの海外ローミングに対応 - KDDI ニュースルーム — 売買指示ではなく確認対象です。
- **WATCH** KDDI、ミリ波を活用したフリーWi-Fiを「ROCK IN JAPAN FESTIVAL 2026」へ商用導入 - ビジネスネットワーク — 売買指示ではなく確認対象です。

## 調査の入口

- 日本：Mito Securities Co.,Ltd.、Ichiyoshi Securities Co.,Ltd.、IwaiCosmo Holdings,Inc.、Akatsuki Inc.
- 米国：Carter Bankshares, Inc. - Common Stock、International Seaways, Inc. Common Stock 、Scorpio Tankers Inc. Common Shares、Millrose Properties, Inc. Class A Common Stock
...
- 上位: ビットコイン 58.9 / スタンダード 57.1 / テクノロジー 54.6
...
データ品質は74.5%。9569銘柄中5366銘柄を採点し、価格欠損率は9.3%です。
公式財務データが不足する場合は、証券会社画面と企業の公式開示を確認するまで注文しません。

生成時刻（UTC）：2026-09-11T07:31:53+00:00
